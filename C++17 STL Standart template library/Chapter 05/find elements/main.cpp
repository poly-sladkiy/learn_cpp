...
bool operator==(const city &a, const city &b) {
    return tie(a.name, a.population) == tie(b.name, b.population);
}
...
int main() {
    ...
    cout << boolalpha << contains_7 << '\n';

    auto [lower_it, upper_it] (
            equal_range(begin(v), end(v), 7)
    );
    print_int(lower_it); // first number not less than 7
    print_int(upper_it); // first number more than 7
